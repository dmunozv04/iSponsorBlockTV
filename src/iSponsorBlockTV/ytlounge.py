...
class iSponsorBlockEventListener(EventListener):
    ...
    async def volume_changed(self, event) -> None:
        ...
        data = {"volume": getattr(event, "volume", 100), "muted": getattr(event, "muted", False)}
    ...
    async def autoplay_up_next_changed(self, event) -> None:
        ...
        data = {"videoId": getattr(event, "video_id", "")}
    ...
    async def subtitles_track_changed(self, event) -> None:
        ...
        data = {"videoId": getattr(event, "video_id", None)}
    ...
    async def playback_speed_changed(self, event) -> None:
        ...
        data = {"playbackSpeed": str(getattr(event, "playback_speed", 1.0))}
            ...
            mock_state = type(
                ...
                (),
                {
                    ...
                    "state": type(
                        ...
                        (),
                        {
                            "value": int(args[0].get("state", 0))
                            if args
                            and isinstance(args[0], dict)
                            and args[0].get("state", "").isdigit()
                            else 0
                        },
                    )(),
                    "currentTime": float(args[0].get("currentTime", 0))
                    if args and isinstance(args[0], dict)
                    else 0.0,
                },
            )()
