...
export const authApi = {
  ...
  verify: () => request<{ username: string; authenticated: boolean }>('/auth/verify'),
...
export const configApi = {
  ...
  getCategories: () => request<import('./types').Category[]>('/config/categories'),
}
...
export const deviceApi = {
  ...
  update: (screenId: string, device: { screen_id: string; name: string; offset: number }) =>
    request<{ message: string }>(`/devices/${encodeURIComponent(screenId)}`, {
      method: 'PUT',
      body: JSON.stringify(device),
    }),
...
export const channelApi = {
  search: (query: string) =>
    request<import('./types').ChannelSearchResult[]>(`/channels/search?q=${encodeURIComponent(query)}`),
}
