...
export default function DashboardPage() {
  ...
  const devicesWithStatus = devices.map((device) => {
    const deviceStatus = status?.devices?.[device.screen_id] as DeviceState | undefined
    return {
      ...device,
  ...
  const connectedCount = devicesWithStatus.filter((d) => d.status === 'connected').length
  ...
  return (
    <div>
      ...
      <Card title="Devices" extra={<Button icon={<ReloadOutlined />} onClick={fetchDevices}>Refresh</Button>}>
        {loading ? (
          ...
          <List
            ...
            renderItem={(device) => (
              <List.Item
                extra={<DeviceStatusTag status={device.status || 'disconnected'} />}
              >
                <List.Item.Meta
